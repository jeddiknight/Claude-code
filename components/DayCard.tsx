...
const dayColors = ["from-blue-500 to-blue-600","from-purple-500 to-purple-600","from-green-500 to-green-600","from-orange-500 to-orange-600","from-red-500 to-red-600","from-teal-500 to-teal-600","from-indigo-500 to-indigo-600","from-pink-500 to-pink-600"];

export default function DayCard({ day, dayIndex }: { day: Day; dayIndex: number }) {
  const [copied, setCopied] = useState(false);
  const [expanded, setExpanded] = useState(true);

  const copyDay = async () => {
    const text = `${day.day} Diena: ${day.title}\n\n${day.locations.map(l=>`• ${l.name}\n  ${l.description}\n  ${l.duration} | ${l.cost}`).join("\n\n")}\n\nPatarimai: ${day.tips}`;
    try {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className={`bg-gradient-to-r ${dayColors[dayIndex % dayColors.length]} px-6 py-5`}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={copyDay} className="bg-white/20 active:bg-white/40 text-white px-3 py-2 rounded-lg text-sm font-medium touch-manipulation min-h-[40px]">
              {copied ? "✓ Nukopijuota" : "📋 Kopijuoti"}
            </button>
            <button onClick={() => setExpanded(!expanded)} className="bg-white/20 active:bg-white/40 text-white w-10 h-10 rounded-lg flex items-center justify-center touch-manipulation text-lg">
              {expanded ? "−" : "+"}
            </button>
          </div>
        </div>
        ...
        <div className="p-6">
                ...
                <div key={i} className="flex gap-4 p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-3 mt-2">
                      <span className="text-xs text-gray-500">⏱ {loc.duration}</span>
                      <span className="text-xs text-gray-500">💶 {loc.cost}</span>
          ...
          {day.tips && (
            <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
              <p className="text-amber-800 text-sm"><span className="font-semibold">💡 Patarimas: </span>{day.tips}</p>
            </div>
          )}
        </div>
      )}
